"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AudioController = void 0;
const common_1 = require("@nestjs/common");
const audio_service_1 = require("./audio.service");
const audio_dto_1 = require("./dto/audio.dto");
let AudioController = class AudioController {
    audioService;
    constructor(audioService) {
        this.audioService = audioService;
    }
    async receiveAudio(dto) {
        return this.audioService.receiveAudio(dto);
    }
    async transcribeAudio(dto) {
        return this.audioService.transcribeAudio(dto);
    }
    async getAudioMessages(chatId, limit) {
        return this.audioService.getAudioMessages(chatId, limit ? parseInt(limit) : 50);
    }
    async getAudioMessage(audioMessageId) {
        return this.audioService.getAudioMessage(audioMessageId);
    }
    async deleteAudio(audioMessageId) {
        await this.audioService.deleteAudio(audioMessageId);
        return { success: true, message: 'Áudio deletado' };
    }
};
exports.AudioController = AudioController;
__decorate([
    (0, common_1.Post)('receive'),
    (0, common_1.HttpCode)(common_1.HttpStatus.CREATED),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [audio_dto_1.ReceiveAudioDto]),
    __metadata("design:returntype", Promise)
], AudioController.prototype, "receiveAudio", null);
__decorate([
    (0, common_1.Post)('transcribe'),
    (0, common_1.HttpCode)(common_1.HttpStatus.OK),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [audio_dto_1.TranscribeAudioDto]),
    __metadata("design:returntype", Promise)
], AudioController.prototype, "transcribeAudio", null);
__decorate([
    (0, common_1.Get)('messages/:chatId'),
    __param(0, (0, common_1.Param)('chatId')),
    __param(1, (0, common_1.Query)('limit')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", Promise)
], AudioController.prototype, "getAudioMessages", null);
__decorate([
    (0, common_1.Get)(':audioMessageId'),
    __param(0, (0, common_1.Param)('audioMessageId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], AudioController.prototype, "getAudioMessage", null);
__decorate([
    (0, common_1.Delete)(':audioMessageId'),
    (0, common_1.HttpCode)(common_1.HttpStatus.OK),
    __param(0, (0, common_1.Param)('audioMessageId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], AudioController.prototype, "deleteAudio", null);
exports.AudioController = AudioController = __decorate([
    (0, common_1.Controller)('audio'),
    __metadata("design:paramtypes", [audio_service_1.AudioService])
], AudioController);
//# sourceMappingURL=audio.controller.js.map